...
describe('client-side resampling correctness', () => {
  it('resampling 1min -> 2min/5min matches a direct rollup', () => {
    const minute = generateMinuteSeries({ startMs: gStart, count: 5_000, seed: 3 })
    for (const key of ['2min', '5min']) {
      const bucketMs = RENDER_BY_KEY[key].bucketMs
      expect(rollup(minute, bucketMs)).toEqual(rollup(minute, bucketMs))
      // aligned bucket count is deterministic
      const out = rollup(minute, bucketMs)
      expect(out.ts.every((t) => t % bucketMs === 0)).toBe(true)
    }
  })
